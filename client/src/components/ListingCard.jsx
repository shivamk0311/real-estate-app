        ...
        <div className="bg-white shadow-md hover:shadow-lg transition-shadow overflow-hidden rounded-lg w-full sm:w-[310px]">
            <Link to={`/listing/${listing._id}`}>
                ...
                <div className="p-3 flex flex-col gap-2 w-full">
                    <p className=" truncate text-lg font-semibold text-amber-950">{listing.name}</p>
                    <div className="flex items-center gap-1">
                        <MdLocationOn className="h-4 w-4 text-green-700"/>
                        <p className="truncate text-sm text-amber-900 w-full">{listing.address}</p>
                    </div>
                    <p className="text-sm text-amber-900 line-clamp-2">{listing.description}</p>
                    ...
                    <div className="text-amber-1000 flex gap-4">
